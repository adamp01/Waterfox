...
find ./ -type f -name "*.exe" -exec java -jar $JSIGN --storetype AZUREKEYVAULT --keystore "$AZURE_VAULT_ID" --alias "$AZURE_CRT" --tsaurl "http://rfc3161timestamp.globalsign.com/advanced" --tsmode RFC3161 --alg SHA-256 --storepass "$(az account get-access-token --resource "https://vault.azure.net" --tenant $AZURE_TENANT_ID | jq -r .accessToken)" {} \;
find ./ -type f -name "*.dll" -exec java -jar $JSIGN --storetype AZUREKEYVAULT --keystore "$AZURE_VAULT_ID" --alias "$AZURE_CRT" --tsaurl "http://rfc3161timestamp.globalsign.com/advanced" --tsmode RFC3161 --alg SHA-256 --storepass "$(az account get-access-token --resource "https://vault.azure.net" --tenant $AZURE_TENANT_ID | jq -r .accessToken)" {} \;
